...
import { ChevronLeft, Clock, AlertTriangle, BookOpen } from "lucide-react";
...
export function InhabitantsStep({ data, onUpdate, onNext, onPrev }: WizardStepProps) {
  ...
  const getAdditionOrder = () => {
    const isSaltwater = data.tankGoal.includes('Saltwater') || data.tankGoal.includes('Reef') || data.tankGoal.includes('FOWLR');
      ...
      return [
        {
          ...
          selectedSpecies: data.selectedSpecies.filter(s => s.toLowerCase().includes('crab') || s.toLowerCase().includes('snail') || s.toLowerCase().includes('shrimp'))
        },
        {
          phase: 'Week 3-4',
          title: 'Hardy Fish',
          ...
          selectedSpecies: data.selectedSpecies.filter(s => s.toLowerCase().includes('clown') || s.toLowerCase().includes('damsel') || s.toLowerCase().includes('goby'))
        },
        {
          phase: 'Week 6-8',
          title: 'Delicate Fish',
          ...
          selectedSpecies: data.selectedSpecies.filter(s => s.toLowerCase().includes('tang') || s.toLowerCase().includes('angel') || s.toLowerCase().includes('wrasse'))
        },
        {
          phase: 'Month 3+',
          ...
          selectedSpecies: data.selectedSpecies.filter(s => s.toLowerCase().includes('coral'))
        }
      ];
    } else {
      return [
        {
          ...
          color: 'bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-700',
          selectedSpecies: data.selectedSpecies.filter(s => s.toLowerCase().includes('tetra') || s.toLowerCase().includes('barb') || s.toLowerCase().includes('danio'))
        },
        {
          phase: 'Week 2-3',
          ...
          color: 'bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-700',
          selectedSpecies: data.selectedSpecies.filter(s => s.toLowerCase().includes('cory') || s.toLowerCase().includes('catfish') || s.toLowerCase().includes('loach'))
        },
        {
          phase: 'Week 4+',
          ...
          recommendedQuantity: '1-2 fish',
          color: 'bg-purple-50 border-purple-200 dark:bg-purple-900/20 dark:border-purple-700',
          selectedSpecies: data.selectedSpecies.filter(s => s.toLowerCase().includes('angel') || s.toLowerCase().includes('gourami') || s.toLowerCase().includes('betta'))
        },
        {
          phase: 'Anytime',
          title: 'Invertebrates',
          ...
          selectedSpecies: data.selectedSpecies.filter(s => s.toLowerCase().includes('shrimp') || s.toLowerCase().includes('snail'))
        }
      ];
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {additionOrder.filter(phase => phase.selectedSpecies.length > 0).map((phase, index) => (
          <Card key={index} className={`vibrant-card ${phase.color}`}>
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground dark:text-slate-300 mb-2">{phase.description}</p>
              
              {phase.selectedSpecies && phase.selectedSpecies.length > 0 && (
                <div className="space-y-2">
                  <p className="text-sm font-medium text-foreground dark:text-slate-200">Your Selected Species:</p>
                  <div className="flex flex-wrap gap-1">
                    {phase.selectedSpecies.map(species => (
                      <Badge key={species} variant="default" className="text-xs">
                        {species}
                      </Badge>
                    ))}
                  </div>
      ...
      <Card className="vibrant-card bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-700">
        <CardHeader>
        ...
        <CardContent className="space-y-3">
          ...
          <Button
            ...
            className="border-blue-300 text-blue-700 hover:bg-blue-100 bg-white/90 backdrop-blur-sm dark:border-blue-600 dark:text-blue-300 dark:hover:bg-blue-800/50"
          >
