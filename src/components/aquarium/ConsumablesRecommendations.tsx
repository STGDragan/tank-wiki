...
export function ConsumablesRecommendations({ aquariumType }: ConsumablesRecommendationsProps) {
  ...
  // Fetch consumable products from database
  const { data: products, isLoading } = useQuery({
    queryKey: ['consumables-recommendations', aquariumType],
    queryFn: async () => {
      let query = supabase
        .from('products')
        .select(`
        ...
        `)
        .eq('visible', true)
        .in('category', ['Food & Nutrition', 'Water Treatment', 'Supplements']); // Consumable categories
  ...
  if (isLoading) {
    return (
      <Card className="mt-6">
        <CardHeader>
          <CardTitle>Loading Consumables Recommendations...</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                <CarouselItem key={product.id} className="md:basis-1/2 lg:basis-1/3 xl:basis-1/4">
                  <Card className="overflow-hidden hover:shadow-lg transition-shadow">
                    ...
                    <CardContent className="p-4">
                      ...
                      {product.description && (
                        <p className="text-xs text-muted-foreground mt-1 line-clamp-3 mb-2">
                          {product.description}
                        </p>
                      )}
